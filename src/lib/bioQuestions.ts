...
export const BIO_QUESTIONS_THERAPEUTIC_FILE = 'bio_questions_therapeutic.json';
...
export function resolveBioQuestionsFile(facultyId: string | null | undefined): string {
  ...
  if (facultyId === 'therapeutic') return BIO_QUESTIONS_THERAPEUTIC_FILE;
  return BIO_QUESTIONS_STOM_FILE;
}

export function isBioFacultyQuestionsFile(fileName: string): boolean {
  return fileName === BIO_QUESTIONS_STOM_FILE
    || fileName === BIO_QUESTIONS_PED_FILE
    || fileName === BIO_QUESTIONS_THERAPEUTIC_FILE;
}
